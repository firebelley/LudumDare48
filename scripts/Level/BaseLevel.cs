...
namespace Game.Level
{
    public class BaseLevel : Node
    {
        ...
        {
            var boundaryEntitiesInRadius = Entities.GetNodesOfType<Building>()
                .Where(x => (x is Tower || x is MainBuilding) && x != tower && GridUtils.IsPointWithinRadius(tower.TilePosition, x.TilePosition, tower.Radius));
            var otherBoundariesConnected = boundaryEntitiesInRadius.All(x => boundaryEntitiesInRadius.Any(y => x != y && GridUtils.IsPointWithinRadius(x.TilePosition, y.TilePosition, x.Radius)));
            if (boundaryEntitiesInRadius.Count() > 1 && !otherBoundariesConnected) return true;
            ...
            var buildingsAreCoveredByOtherTower = buildingsInRadius.All(x => boundaryEntitiesInRadius.Any(y => GridUtils.IsPointWithinRadius(y.TilePosition, x.TilePosition, y.Radius)));
